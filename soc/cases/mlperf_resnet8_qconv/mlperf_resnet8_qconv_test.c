...
#include "mlperf_resnet8_qconv_test.h"
...
int main(void) {
    uint32_t insn = (uint32_t)(uintptr_t)mlperf_resnet8_qconv_insn;
    uint32_t uop = (uint32_t)(uintptr_t)mlperf_resnet8_qconv_uop;
    uint32_t inp = (uint32_t)(uintptr_t)mlperf_resnet8_qconv_inp;
    uint32_t wgt = (uint32_t)(uintptr_t)mlperf_resnet8_qconv_wgt;
    uint32_t acc = (uint32_t)(uintptr_t)mlperf_resnet8_qconv_acc;
    uint32_t out = (uint32_t)(uintptr_t)mlperf_resnet8_qconv_out;

    printf("VTA case mlperf_resnet8_qconv: insn=%u\r\n", (unsigned int)MLPERF_RESNET8_QCONV_INSN_COUNT);
    ...
    MMIO32(VTA_INSN_COUNT_REG) = MLPERF_RESNET8_QCONV_INSN_COUNT;
    ...
    for (size_t offset = 0; offset < MLPERF_RESNET8_QCONV_EXPECTED_SIZE; offset += 4U) {
        uint32_t actual = *(volatile uint32_t *)((uintptr_t)mlperf_resnet8_qconv_out + offset);
        uint32_t expected = *(const uint32_t *)((uintptr_t)mlperf_resnet8_qconv_expected + offset);
        if (actual != expected) {
